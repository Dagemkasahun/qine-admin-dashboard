...
// Get all orders (with optional rider filter)
...
app.get('/api/riders/:id', async (req, res) => {
  try {
    ...
    const completedOrders = await prisma.order.count({
      where: { riderId: rider.userId, status: 'DELIVERED' }
    });
    
    // Get total earnings from delivery fees
    ...
    res.json({
      ...
      totalEarnings: earningsResult._sum.deliveryFee || 0,
    });
  } catch (error) {
